...
public abstract class AbstractDatabaseObservationDAOTest
{
    ...
    public void testGetObservationStateList()
    {
        try
        {
            ...
            List<ObservationState> result = dao.getObservationStates(collection, start, end, batchSize);
            Assert.assertEquals(4, result.size());
            for (ObservationState os : result)
                log.info("found: " + os);
            
            result = dao.getObservationStates(collection, start, mid, batchSize);
            Assert.assertEquals(2, result.size());
            for (ObservationState os : result)
                log.info("found: " + os);
            
            result = dao.getObservationStates(collection, mid, end, batchSize);
            Assert.assertEquals(2, result.size());
            for (ObservationState os : result)
                log.info("found: " + os);
            ...
            try
            {
                result = dao.getObservationStates(null, start, end, batchSize);
            ...
            result = dao.getObservationStates(collection, null, end, batchSize);
            Assert.assertEquals(4, result.size());
            
            result = dao.getObservationStates(collection, start, null, batchSize);
            Assert.assertEquals(4, result.size());
            
            result = dao.getObservationStates(collection, null, null, batchSize);
            
            result = dao.getObservationStates(collection, start, null, null);
        }            
        catch(Exception unexpected)
        {
            log.error("unexpected exception", unexpected);
    ...
    public void testPutSimpleObservation()
    {
        try
        {
            ...
            {
                for (boolean sci : new boolean[] { false, true })
                {
    ...
    private Plane getTestPlane(boolean full, String productID, int depth)
        throws Exception
    {
