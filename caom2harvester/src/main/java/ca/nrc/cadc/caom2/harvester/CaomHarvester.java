...
public class CaomHarvester implements Runnable {
     ...
     */
    public CaomHarvester(boolean dryrun, String[] src, String[] dest, int batchSize,
        ...
        this.observationMetaHarvester = new ReadAccessHarvester(ObservationMetaReadAccess.class,
                src, dest, entityBatchSize, full, dryrun);
        observationMetaHarvester.setSkipped(skip);
        this.planeDataHarvester = new ReadAccessHarvester(PlaneDataReadAccess.class, src, dest,
                entityBatchSize, full, dryrun);
        planeDataHarvester.setSkipped(skip);
        this.planeMetaHarvester = new ReadAccessHarvester(PlaneMetaReadAccess.class, src, dest,
                entityBatchSize, full, dryrun);
        planeMetaHarvester.setSkipped(skip);

        if (!full) {
            this.obsDeleter = new DeletionHarvester(DeletedObservation.class, src, dest,
                    entityBatchSize, dryrun);

            if (!skip) {
                this.observationMetaDeleter = new DeletionHarvester(
                        DeletedObservationMetaReadAccess.class, src, dest, entityBatchSize, dryrun);
                this.planeMetaDeleter = new DeletionHarvester(DeletedPlaneMetaReadAccess.class, src,
                        dest, entityBatchSize, dryrun);
                this.planeDataDeleter = new DeletionHarvester(DeletedPlaneDataReadAccess.class, src,
                        dest, entityBatchSize, dryrun);
            }
        }
    }

    public CaomHarvester(boolean dryrun, String[] src, String[] dest, Integer batchSize,
    ...
    public static CaomHarvester getTestHarvester(boolean dryrun, String[] src, String[] dest,
            ...
            throws IOException, URISyntaxException {
        CaomHarvester ret = new CaomHarvester(dryrun, src, dest, batchSize, batchFactor, full, skip,
                maxdate);
    ...
    public void run() {
        ...
        if (observationMetaDeleter != null) {
            ...
            if (!init) {
                ...
                HarvestState hs = observationMetaHarvester.harvestState
                        .get(observationMetaHarvester.source, observationMetaHarvester.cname);
                initDel = (hs.curID == null && hs.curLastModified == null); // never harvested from
                                                                            // source before
            }
        ...
        if (obsDeleter != null) {
            ...
            if (!init) {
                // check if we have ever harvested before
                HarvestState hs = obsHarvester.harvestState.get(obsHarvester.source,
                        obsHarvester.cname);
                initDel = (hs.curID == null && hs.curLastModified == null); // never harvested from
                                                                            // source before
            }
