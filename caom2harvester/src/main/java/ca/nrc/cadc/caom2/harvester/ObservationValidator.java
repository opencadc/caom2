...
public class ObservationValidator extends Harvester
{
            ...
            List<ObservationState> tmpDstState, List<Observation> tmpSrc, List<Observation> tmpDst)
    {
        ...
        for (int i = 0; i < tmpSrcState.size(); i++)
        {
            ...
            ObservationState osDst = tmpDstState.get(i);
            Observation oDst = tmpDst.get(i);

            if (!osSrc.getURI().getObservationID().equals(oSrc.getURI().getObservationID())
                    || !oSrc.getURI().getObservationID().equals(osDst.getURI().getObservationID())
                    || !osDst.getURI().getObservationID().equals(oDst.getURI().getObservationID())
                    || !osSrc.getURI().getObservationID().equals(oDst.getURI().getObservationID()))
            {
            ...
            else
            {
                if (!(osSrc.accMetaChecksum != null && osDst.accMetaChecksum != null
                        && oSrc.getAccMetaChecksum() != null && oDst.getAccMetaChecksum() != null
                        && osSrc.accMetaChecksum.equals(oSrc.getAccMetaChecksum())
                        && oSrc.getAccMetaChecksum().equals(osDst.accMetaChecksum)
                        && osDst.accMetaChecksum.equals(oDst.getAccMetaChecksum())
                        && oDst.getAccMetaChecksum().equals(osSrc.accMetaChecksum)))
                {
