...
public class ArtifactDAO extends AbstractCaomEntityDAO<Artifact> {
    ...
    // stand-alone query use: read grant for artifact
    public ArtifactDAO(boolean origin) {
        super(origin);
    }
    
    public ArtifactDAO(AbstractCaomEntityDAO dao) {
        super(dao);
        this.partDAO = new PartDAO(dao);
    }

    // use case: caom2-artifact-discover and caom2-artifact-validate
    public ResourceIterator<Artifact> iterator(String namespace, String uriBucket, 
    ...
    @Override
    public void put(Skeleton cur, Artifact a, Entity parent, JdbcTemplate jdbc) {
        if (a == null) {
        ...
        try {
            ...
            super.put(cur, a, parent, jdbc);

            for (Pair<Part> p : pairs) {
                partDAO.put(p.cur, p.val, a, jdbc);
            }
        } finally {
