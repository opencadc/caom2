...
public class HarvestSkipDAO {
    ...
    private static final String[] COLUMNS = {
        "source", "cname", "uri", "uriBucket", 
        "tryAfter", "errorMessage", 
        "lastModified", "id" // PK
    };
    ...
    public HarvestSkipDAO(DataSource dataSource, String database, String schema) {
        ...
        sb.append(HarvestSkip.class.getSimpleName());
    ...
    public List<HarvestSkip> get(String source, String cname, Date start, Date end, Integer batchSize) {
        ...
        List result = jdbc.query(sel, extractor);
        List<HarvestSkip> ret = new ArrayList<>(result.size());
        for (Object o : result) {
     ...
     */
    public ResourceIterator<HarvestSkip> iterator(String name, String namespace, String bucketPrefix, Date maxTryAfter) {
    ...
    public ResourceIterator<HarvestSkip> iterator(String name, String namespace, 
    ...
    private class PutStatementCreator implements PreparedStatementCreator {
        ...
        private void loadValues(PreparedStatement ps)
            ...
            if (skip.tryAfter != null) {
                ps.setTimestamp(col++, new Timestamp(skip.tryAfter.getTime()), utcCalendar);
            } else {
            ...
            ps.setString(col++, es);
            ps.setTimestamp(col++, new Timestamp(skip.lastModified.getTime()), utcCalendar);
    ...
    private class HarvestSkipMapper implements RowMapper {
        ...
        public HarvestSkip mapRow(ResultSet rs, int i) throws SQLException {
            ...
            HarvestSkip ret = new HarvestSkip(source, name, uri);
            
            ret.tryAfter = Util.getDate(rs, col++, utc);
    ...
    class IteratorQuery {
        ...
        public ResourceIterator<HarvestSkip> query(DataSource ds) {
            
            StringBuilder sb = new StringBuilder(SqlUtil.getSelectSQL(COLUMNS, tableName));
            sb.append(" WHERE cname = ?");
            
            if (namespace != null) {
                sb.append(" AND uri LIKE ?");
            }
            if (bucketPrefix != null) {
                sb.append(" AND uriBucket LIKE ?");
            } else if (minBucket != null && maxBucket != null) {
                sb.append(" AND uriBucket BETWEEN ? AND ?");
            }
            if (maxTryAfter != null) {
    ...
    private class ResultSetIterator implements ResourceIterator<HarvestSkip> {
        ...
        @Override
        public HarvestSkip next() {
            if (!hasRow) {
                throw new NoSuchElementException();
            }
            try {
                HarvestSkip ret = mapper.mapRow(rs, rowNum++);
                hasRow = rs.next();
                if (!hasRow) {
