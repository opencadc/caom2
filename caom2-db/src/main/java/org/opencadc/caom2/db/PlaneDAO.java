...
class PlaneDAO extends AbstractCaomEntityDAO<Plane> {
    ...
    PlaneDAO(AbstractCaomEntityDAO dao) {
        super(dao);
        this.artifactDAO = new ArtifactDAO(dao);
    }

    @Override
    public void put(Skeleton cur, Plane p, Entity parent, JdbcTemplate jdbc) {
        if (p == null) {
        ...
        try {
            ...
            for (Pair<Artifact> a : pairs) {
                artifactDAO.put(a.cur, a.val, p, jdbc);
            }
        } finally {
