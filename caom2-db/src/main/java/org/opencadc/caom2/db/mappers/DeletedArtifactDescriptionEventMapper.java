 ...
 * @author pdowler
 */
public class DeletedArtifactDescriptionEventMapper implements RowMapper<DeletedArtifactDescriptionEvent> {
    private static final Logger log = Logger.getLogger(DeletedArtifactDescriptionEventMapper.class);
    private final Calendar utcCalendar = Calendar.getInstance(DateUtil.UTC);    

    public static final String[] COLUMNS = new String[] {
        "uri", "lastModified", "metaChecksum", "metaProducer", 
        "id" // PK
    };

    public DeletedArtifactDescriptionEventMapper() { 
    }
    
    public DeletedArtifactDescriptionEvent mapRow(ResultSet rs, int row)
            throws SQLException {
        int col = 1;
        URI uri = Util.getURI(rs, col++);
        Date lastModified = Util.getDate(rs, col++, utcCalendar);
        URI metaChecksum = Util.getURI(rs, col++);
        URI metaProducer = Util.getURI(rs, col++);
        UUID id = Util.getUUID(rs, col++);
        DeletedArtifactDescriptionEvent ret = new DeletedArtifactDescriptionEvent(id, uri);
