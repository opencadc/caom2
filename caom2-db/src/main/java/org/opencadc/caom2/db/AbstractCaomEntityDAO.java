...
/**
 * Base class for CaomEntity DAO classes.
 * 
 * @author pdowler
 */
public abstract class AbstractCaomEntityDAO<T extends CaomEntity> extends AbstractDAO {
    private static final Logger log = Logger.getLogger(AbstractCaomEntityDAO.class);

    protected AbstractCaomEntityDAO(boolean origin) {
        super(origin);
    }

    protected AbstractCaomEntityDAO(AbstractDAO dao) {
        super(dao);
    }
    ...
    protected void put(Skeleton cur, T val, Entity parent, JdbcTemplate jdbc) {
        put(cur, val, parent, jdbc, false);
    }

    protected void put(Skeleton cur, T val, Entity parent, JdbcTemplate jdbc, boolean force) {
        if (readOnly) {
