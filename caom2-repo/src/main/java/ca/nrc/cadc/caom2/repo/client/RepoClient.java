...
public class RepoClient
{
    ...
    public List<ObservationState> getObservationList(String collection, Date start, Date end, Integer maxrec) throws AccessControlException
    {
        ...
        TreeSet<ObservationState> accSet = new TreeSet<ObservationState>(uriComparator);
        List<ObservationState> partialSet = null;
        ...
        while (go)
        {
            ...
            try
            {
                partialSet = transformByteArrayOutputStreamIntoListOfObservationState(bos, df, '\t', '\n');
                accSet.addAll(partialSet);
                bos.close();
            }
            ...
            start = accSet.last().maxLastModified;
            recCounter += partialSet.size();
            if (partialSet.size() == 1 && (maxrec != null && recCounter >= maxrec) || (start != null && end != null && start.equals(end)) || partialSet.size() < rec)
            {
                go = false;
            }
        }
