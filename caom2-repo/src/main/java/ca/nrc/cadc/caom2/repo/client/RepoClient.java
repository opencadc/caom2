    ...
    public List<ObservationState> getObservationList(String collection, Date start, Date end, Integer maxrec) throws AccessControlException
    {
        ...
        while (go)
        {
            ...
            try
            {
                ...
                if (partialList != null && accList != null && partialList.size() >= 1 && accList.size() >= 1 && partialList.get(partialList.size() - 1) != null
                        && partialList.get(partialList.size() - 1).equals(accList.get(0)))
                {
            ...
            if (partialList.size() == 0 && (maxrec != null && recCounter >= maxrec) || (start != null && end != null && start.equals(end)) || partialList.size() < rec)
            {
                go = false;
            }
        }
