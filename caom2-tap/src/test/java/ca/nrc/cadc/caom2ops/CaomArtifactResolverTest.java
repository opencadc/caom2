...
public class CaomArtifactResolverTest {
    ...
    public void testCutoutURL() {
        try {
            ...
            URI uri;
            URL url;
            List<String> cutouts = new ArrayList<>();
    ...
    public void testAuthMethod() {
        try {
            CaomArtifactResolver car = new CaomArtifactResolver();
            car.setAuthMethod(AuthMethod.PASSWORD);
            ...
            Assert.assertTrue("URL should contain /auth for a Password authmethod.",
                              url.toExternalForm().contains("/auth"));

            car.setAuthMethod(AuthMethod.CERT);

            uri = URI.create("ad:BAR/foo");
            url = car.getURL(uri);

            Assert.assertEquals("Scheme should be https for cert method.", "https",
                                url.getProtocol());
