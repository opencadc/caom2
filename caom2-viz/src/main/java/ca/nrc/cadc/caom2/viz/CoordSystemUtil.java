...
public static void setLowerLeftCoordSystem(Graphics2D g, Component comp,
  double unit) { setLowerLeftCoordSystem(g, comp, unit, unit); }
...
public static void setLowerCenteredCoordSystem(Graphics2D g, Component comp,
  ...
  g.translate(dx/2, dy); g.scale(unitx, -unity); g.translate(0, 1); }
...
public static void setLowerCenteredCoordSystem(Graphics2D g, Component comp,
  double unit) { setLowerCenteredCoordSystem(g, comp, unit, unit); }
...
public static void setLowerCenteredCoordSystem(Graphics2D g,
  double unit) { setUpperLeftCoordSystem(g, unit, unit); }
...
public static void setCenteredCoordSystem(Graphics2D g, Component comp,
  ...
  int dx = comp.getWidth();
  int dy = comp.getHeight();
  g.translate(dx/2, dy/2); g.scale(unitx, -unity); }
...
public static void setCenteredCoordSystem(Graphics2D g, Component comp,
  ...
  int dx = comp.getWidth();
  int dy = comp.getHeight();
  g.translate(dx/2, dy/2); g.scale(unit, -unit); }
...
public static void setCenteredCoordSystem(Graphics2D g, Component comp) {
  int dx = comp.getWidth();
  int dy = comp.getHeight();
  g.translate(dx/2, dy/2); g.scale(CM, -CM); }
...
double unit) {
  //int dx = comp.getWidth();
  int dy = comp.getHeight();
  g.translate(0, dy/2); g.scale(unit, -unit);
  //g.translate(1,0);
  }
...
public static void setLeftCenteredCoordSystem(Graphics2D g, Component comp) {
  //int dx = comp.getWidth();
  int dy = comp.getHeight();
  g.translate(0, dy/2); g.scale(CM, -CM);
  //g.translate(1,0);
  }

  /**
   * constructs a grid covering a rectangular area on the screen.
   * @param xL the lower boundary of the x interval
   * @param xH the upper boundary of the x interval
   * @param nX the number of subintervals of the x interval
   * @param yL the lower boundary of the y interval
   * @param yH the upper boundary of the y interval
   * @param nY the number of subintervals of the y interval
   * @return the GeneralPath object representing the grid
   */

public static GeneralPath grid(
  double xL, double xH, int nX, //x interval
  double yL, double yH, int nY) { //y interval {
  double dx = (xH-xL)/nX;
  double dy = (yH-yL)/nY;
  double x = xL;
  double y = yL;
  ...
  for (int i = 0; i <= nX; i++)
  {
    gp.moveTo((float) x, (float) yL);
    gp.lineTo((float) x, (float) yH); x += dx;
  }
  for (int j = 0; j <= nY; j++)
  {
    gp.moveTo((float) xL, (float) y);
    gp.lineTo((float) xH, (float) y); y += dy;
  }
  return gp; }
...
public static void drawGrid(Graphics2D g,
    double xL, double xH, int nX, //x interval
    double yL, double yH, int nY) {
  GeneralPath gp = grid(xL, xH, nX, yL, yH, nY);
  g.draw(gp); }
...
public static void drawIntegralCoordinateGrid(Graphics2D g, Component comp, int delta) {
  ...
  for (int i = 0; i < 2; i++)
  { q[i] = new Point2D.Double(); bt.transform(p[i], q[i]); }
  double xL = q[0].x;
  double xH = q[1].x;
  double y0 = q[0].y;
  double y1 = q[1].y;
  double yL = y0 < y1 ? y0 : y1;
  double yH = y0 < y1 ? y1 : y0;
  GeneralPath gp = new GeneralPath();
  double x;
  double y;
  x = Math.ceil(xL);
...
public static void drawCoordinateAxes(Graphics2D g, Component comp)  {
  ...
  GeneralPath gp = new GeneralPath();
  gp.moveTo((float) xL, 0f);
  gp.lineTo((float) xH, 0f);
  gp.moveTo(0f, (float) yL);
  gp.lineTo(0f, (float) yH);
  g.draw(gp); }
...
public static void setFittedCoordSystem(Graphics2D g, Component comp,
  double xL, double xH, double yL, double yH, double r) {
    double xxL = xL-r;
    double xxH = xH+r;
    double yyL = yL-r;
    double yyH = yH+r;
...
public static void setFittedCoordSystem(Graphics2D g, Component comp,
  Rectangle2D rek) {
  double xL = rek.getX();
  double xH = xL + rek.getWidth();
  double yL = rek.getY();
  double yH = yL + rek.getHeight();
  setFittedCoordSystem(g, comp, xL, xH, yL, yH); }
...
public static Rectangle2D getBounds2D(Shape[] sh) {
   ...
   if ( n > 1) for (int i = 1; i < n; i++) bigrek.add(sh[i].getBounds2D());
   return bigrek; }
...
public static Rectangle2D getBounds2D(List shapes) 
{
    ...
    if ( n > 1) for (int i = 1; i < n; i++) bigrek.add(((DrawArea.CShape) shapes.get(i)).shape.getBounds2D());
    return bigrek;
}
...
public static void setFittedEvenCoordSystem(Graphics2D g, Component comp,
  ...
  double kW = (1.0 * dyW)/dxW;
  double kG = deltay/deltax;
  //double k = kW/kG;
  Rectangle2D re = new Rectangle2D.Double();
  if (kW > kG)
    re.setRect(x, y, deltax, kW*deltax);
  else
    re.setRect(x, y, deltay/kW, deltay);
  setFittedCoordSystem(g, comp, re); }
