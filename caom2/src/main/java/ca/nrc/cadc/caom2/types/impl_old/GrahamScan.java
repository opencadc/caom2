...
public class GrahamScan implements Serializable
{
    ...
    public GrahamScan(SortablePoint2D[] pts) {
        ...
        // defensive copy
        int N = pts.length;
        SortablePoint2D[] points = new SortablePoint2D[N];
        for (int i = 0; i < N; i++)
            points[i] = pts[i];
        ...
        int k2;
        for (k2 = k1 + 1; k2 < N; k2++)
            if (SortablePoint2D.ccw(points[0], points[k1], points[k2]) != 0) break;
    ...
    public Iterable<SortablePoint2D> hull() {
        ...
        for (SortablePoint2D p : hull) s.push(p);
        return s;
    }
    ...
    private boolean isConvex() {
        ...
        for (int i = 0; i < N; i++) {
            if (SortablePoint2D.ccw(points[i], points[(i+1) % N], points[(i+2) % N]) <= 0) {
                return false;
            }
        }
        return true;
