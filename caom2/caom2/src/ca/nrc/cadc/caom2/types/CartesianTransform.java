...
public class CartesianTransform implements Serializable
{
        ...
        if (Math.abs(cz) > 0.001 || force)
        {
            ...
            if (cz > 0.0) // north
            {
               // angle to +X axis, then add 180 degrees
                ...
                trans.a = Math.acos(cosa) + Math.PI;
            }
            else // south
            {
    ...
    public boolean isNull() { return axis == null; }
     ...
     * as long as the opposite conversion is done with toLongLat that is just fine.
     * @param longitude
     * @param latitude
     * @return a double[3]
     */
    ...
    public static double[] toLongLat(double x, double y, double z)
    {
        ...
        if ((x == 0.0 && y == 0.0) || z == 1.0 || z == -1.0) // at pole
            ret[0] = 0.0;
        else
    ...
    {
        double[] ret = new double[3];
        double cr = Math.cos(a);
        double sr = Math.sin(a);
        ret[0] = cr * p[0] + sr * p[2];
        ret[1] = p[1];
