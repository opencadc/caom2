 ...
 * @author pdowler
 */
public class MetadataCompute {
    private static final Logger log = Logger.getLogger(MetadataCompute.class);

    private final Observation obs;
    
    public MetadataCompute(Observation obs) { 
        this.obs = obs;
    }
    ...
    public void run() {
        ...
        try {
            ...
            log.error("failed to compute metadata for plane " + cur, er);
        } catch (Exception ex) {
            log.error("failed to compute metadata for plane " + cur, ex);
        }
    }
}
